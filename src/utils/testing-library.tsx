...
import { Mainsail } from '@/lib/mainsail';
...
import { connectedTransport as ledgerTransportFactory } from '@/lib/contexts/Ledger/transport';
import { useProfileSynchronizer } from '@/lib/hooks/use-profile-synchronizer';
...
export function renderWithForm(
    ...
    options?: {
        ...
        defaultValues?: any;
        registerCallback?: (useFormReturn: UseFormReturn) => void;
    ...
    if (options?.breakpoint) {
        responsiveRenderFunction =
            (options.withProviders ?? true) ? renderResponsiveWithRoute : renderResponsive;
    } else {
...
const renderWithRouter = (
    ...
    const ProfileSynchronizerWrapper = ({ children }: { children: React.ReactNode }) =>
        withProfileSynchronizer ? (
            <ProfileSynchronizer options={profileSynchronizerOptions}>
                {children}
            </ProfileSynchronizer>
        ) : (
            <>{children}</>
...
export const getDefaultWalletId = () =>
    Object.keys(Object.values(fixtureData.profiles)[0].wallets)[0];
export const getDefaultMainsailWalletId = () =>
    Object.keys(Object.values(fixtureData.profiles)[0].wallets)[0];
...
export const getDefaultPassword = () =>
    TestingPasswords.profiles[getPasswordProtectedProfileId()]?.password;
...
export const syncValidators = async (profile: Contracts.IProfile) =>
...
export const renderResponsiveWithRoute = (
    ...
    return renderWithRouter(
        <ResponsiveContext.Provider value={{ width: widths[breakpoint] }}>
            {component}
        </ResponsiveContext.Provider>,
        options,
    );
...
export const queryElementForSvg = (target: HTMLElement, svg: string) =>
    target.querySelector(`svg#${svg}`);
...
export const createTransactionMock = (
    ...
    vi.spyOn(wallet.transaction(), 'transaction').mockReturnValue({
        ...
        id: () => transactionFixture.data.hash,
        isConfirmed: () => true,
        ...
        recipient: () => transactionFixture.data.to,
        recipients: () => [
            {
        ...
        sender: () => transactionFixture.data.from,
        timestamp: () => DateTime.make(),
        total: () => +transactionFixture.data.value / 1e8 + +transactionFixture.data.fee / 1e8,
        type: () => 'transfer',
        wallet: () => wallet,
        ...overrides,
    } as any);
...
export const createMainsailTransactionMock = (
    ...
    vi.spyOn(wallet.transaction(), 'transaction').mockReturnValue({
        ...
        explorerLink: () =>
            `https://mainsail-explorer.ihost.org/transactions/${mainsailTransactionFixture.data.hash}`,
        explorerLinkForBlock: () =>
            `https://mainsail-explorer.ihost.org/transactions/${mainsailTransactionFixture.data.hash}`,
        fee: () =>
            (+mainsailTransactionFixture.data.gasPrice * +mainsailTransactionFixture.data.gas) /
            1e18,
