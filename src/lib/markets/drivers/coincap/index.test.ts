...
describe('CoinCap', () => {
    ...
    it('should verify a token', async () => {
        ...
        server.use(
            requestMock('https://api.coincap.io/v2/assets/ark', { data: assetsFixture.data[0] }),
        );
    ...
    it('should throw for invalid market data token', async () => {
        ...
        const tracker = new CoinCap(new Http.HttpClient(0));
        await expect(tracker.marketData('invalid')).rejects.toThrow(
            'Failed to determine the token.',
        );
    });
    ...
    it('should throw when getting historical volume', async () => {
        const tracker = new CoinCap(new Http.HttpClient(0));
        await expect(tracker.historicalVolume()).rejects.toThrow(
            'Method CoinCap#historicalVolume is not implemented.',
        );
    });
