...
describe('CryptoCompare', () => {
    ...
    it('should fail to verify a token', async () => {
        server.use(
            requestMock('https://min-api.cryptocompare.com/data/price', {}, { status: 500 }),
        );
    ...
    it('should get historical price data', async () => {
        server.use(
            requestMock('https://min-api.cryptocompare.com/data/v2/histoday', histodayFixture),
        );
        const tracker = new CryptoCompare(new Http.HttpClient(0));
