...
export const truncate = (
    ...
    if (options.omissionPosition === 'right') {
        return (
            value.slice(0, Math.max(0, options.length - options.omission.length)) + options.omission
        );
    }
    ...
    return (
        options.omission +
        value.slice(Math.max(0, value.length - options.length + options.omission.length))
    );
};
