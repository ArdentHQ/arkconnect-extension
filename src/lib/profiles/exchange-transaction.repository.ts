...
export class ExchangeTransactionRepository implements IExchangeTransactionRepository {
    ...
    public create(data: IExchangeTransactionInput): IExchangeTransaction {
        ...
        for (const exchangeTransaction of exchangeTransactions) {
            if (
                exchangeTransaction.orderId() === data.orderId &&
                exchangeTransaction.provider() === data.provider
            ) {
                throw new Error(
                    `The exchange transaction [${data.provider} / ${data.orderId}] already exists.`,
                );
            }
        }
    ...
    public pending(): IExchangeTransaction[] {
        return this.values().filter((exchangeTransaction: IExchangeTransaction) =>
            exchangeTransaction.isPending(),
        );
    }
