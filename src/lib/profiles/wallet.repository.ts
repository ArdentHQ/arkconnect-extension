...
export class WalletRepository implements IWalletRepository {
    ...
    public findByAddressWithNetwork(
        address: string,
        network: string,
    ): IReadWriteWallet | undefined {
        return this.values().find(
            (wallet: IReadWriteWallet) =>
                wallet.address() === address && wallet.networkId() === network,
        );
    }
    ...
    public findByAlias(alias: string): IReadWriteWallet | undefined {
        return this.values().find(
            (wallet: IReadWriteWallet) =>
                (wallet.settings().get<string>(WalletSetting.Alias) || '').toLowerCase() ===
                alias.toLowerCase(),
        );
    }
    ...
    public push(wallet: IReadWriteWallet, options?: { force: boolean }): IReadWriteWallet {
        ...
        if (!force && this.findByAddressWithNetwork(wallet.address(), wallet.networkId())) {
            throw new Error(
                `The wallet [${wallet.address()}] with network [${wallet.networkId()}] already exists.`,
            );
        }
        ...
        const syncWallets = (wallets: object): Promise<IReadWriteWallet[]> =>
            pqueue(
                Object.values(wallets).map(
                    (wallet) => () => this.#restoreWallet(wallet, { ttl: options?.ttl }),
                ),
            );
