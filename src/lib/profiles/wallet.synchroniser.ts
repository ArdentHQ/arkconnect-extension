...
export class WalletSynchroniser implements IWalletSynchroniser {
    ...
    public async identity(): Promise<void> {
        ...
        const walletIdentifier: Services.WalletIdentifier = WalletIdentifierFactory.make(
            this.#wallet,
        );
        ...
        try {
             ...
             *A missing wallet could mean that the wallet is legitimatebut has no transactions or that the address is wrong.
             */
    ...
    public async votes(): Promise<void> {
        const { available, votes, used } = await this.#wallet
            .client()
            .votes(this.#wallet.address());
